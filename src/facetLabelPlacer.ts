    ...
    for (const f of facetResult.facets) {
      if (f != null) {
        ...
        const insideFacets = facetResult.facets.filter((facet) => {
          ...
          return FacetLabelPlacer.doesNeighbourFallInsideInCurrentFacet(
            facetPath,
            f,
            onlyOuterRing
          );
        ...
        insideFacets.forEach((facet) => {
          if (facet) {
            ...
            const otherInsideFacets = insideFacets.filter(
              (otherInsideFacet) =>
                otherInsideFacet && otherInsideFacet.id != facet.id
            );
            otherInsideFacets.forEach((otherInsideFacet) => {
              if (otherInsideFacet) {
                const otherInsidePath =
                  otherInsideFacet.getFullPathFromBorderSegments(true);
                if (otherInsideFacet.neighbourFacetsIsDirty)
                  FacetCreator.buildFacetNeighbour(
                    otherInsideFacet,
                    facetResult
                  );
                if (
